...
export default function DepositAmountScreen({ navigation, route }: any) {
    const { colorScheme } = useColorScheme();
    const method = route.params?.method || 'MoMo';
    ...
    return (
        <SafeAreaView className="flex-1 bg-background-light dark:bg-background-dark">
            ...
            {/* Header */}
            <View className="flex-row items-center justify-between px-4 pt-4 pb-2 sticky top-0 z-10 bg-background-light dark:bg-background-dark">
                <TouchableOpacity onPress={() => navigation.goBack()} className="p-1">
                    <MaterialIcons name="arrow-back" size={32} color={colorScheme === 'dark' ? '#fff' : '#1F2937'} />
                </TouchableOpacity>
                <Text className="flex-1 text-center text-lg font-bold text-text-dark-gray dark:text-white pr-10">
                    Nạp tiền vào ví
                </Text>
                <View className="w-10" />
            </View>
