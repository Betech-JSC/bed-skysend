...
export default function ReportIssueScreen({ navigation }: any) {
    ...
    return (
        <SafeAreaView className="flex-1 bg-background-light dark:bg-background-dark">
            ...
            {/* Header */}
            <View className="flex-row items-center justify-between px-4 pt-4 pb-3 shadow-sm">
                <TouchableOpacity
                    onPress={() => navigation.goBack()}
                    className="h-10 w-10 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700"
                >
                    <MaterialIcons name="arrow-back" size={24} color={colorScheme === 'dark' ? '#fff' : '#1F2937'} />
                </TouchableOpacity>

                <Text className="flex-1 text-center text-xl font-bold text-text-primary dark:text-white -ml-10">
                    Báo cáo sự cố
                </Text>
                <View className="w-10" />
            </View>
                ...
                <View className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm space-y-6">
                    ...
                    <View>
                        ...
                        <View className="relative">
                            <View className="absolute right-4 top-4 z-10 pointer-events-none">
                                <MaterialIcons name="keyboard-arrow-down" size={24} color="#616e89" />
                            </View>
                            <select
                                value={issueType}
                                onChange={(e) => setIssueType(e.target.value)}
                                className="w-full h-14 px-4 pr-12 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg text-text-primary dark:text-white text-base appearance-none focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary"
                            >
                                {issueTypes.map((type) => (
